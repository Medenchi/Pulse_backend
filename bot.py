...
WEBAPP_URL = os.environ.get("WEBAPP_URL", "https://placeholder.example.com")
HOST = "0.0.0.0"
...
async def init_db():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS users (
            user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, topic_id INTEGER,
            is_blocked INTEGER DEFAULT 0, message_count INTEGER DEFAULT 0,
        ...
        await db.execute("""CREATE TABLE IF NOT EXISTS portfolio (
            ...
            description TEXT, category TEXT DEFAULT 'general', added_by INTEGER,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
        await db.execute("""CREATE TABLE IF NOT EXISTS orders (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
            order_type TEXT NOT NULL, payment_type TEXT NOT NULL, details TEXT,
            status TEXT DEFAULT 'processing', total_price REAL DEFAULT 0,
            contact_info TEXT DEFAULT '', delivery_info TEXT DEFAULT '',
            admin_note TEXT DEFAULT '',
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
        await db.execute("""CREATE TABLE IF NOT EXISTS message_log (
...
async def get_user(user_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        c = await db.execute("SELECT * FROM users WHERE user_id=?", (user_id,))
        r = await c.fetchone()
...
async def create_user(user_id, username, full_name, topic_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO users (user_id,username,full_name,topic_id,created_at,last_active) "
...
async def get_user_by_topic(topic_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        c = await db.execute("SELECT * FROM users WHERE topic_id=?", (topic_id,))
        r = await c.fetchone()
        return dict(r) if r else None
...
async def get_all_users():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        c = await db.execute("SELECT * FROM users WHERE is_blocked=0 ORDER BY last_active DESC")
...
async def add_portfolio_item(filename, title, description, category, added_by):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        c = await db.execute(
            "INSERT INTO portfolio (filename,title,description,category,added_by) VALUES (?,?,?,?,?)",
            (filename, title, description, category, added_by),
        )
        await db.commit()
        return c.lastrowid
...
async def get_portfolio_items():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        c = await db.execute("SELECT * FROM portfolio ORDER BY created_at DESC")
        return [dict(r) for r in await c.fetchall()]
...
async def get_portfolio_item(item_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        c = await db.execute("SELECT * FROM portfolio WHERE id=?", (item_id,))
        r = await c.fetchone()
        return dict(r) if r else None
...
async def delete_portfolio_item(item_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        c = await db.execute("SELECT filename FROM portfolio WHERE id=?", (item_id,))
        r = await c.fetchone()
        if r:
            fp = os.path.join(ASSETS_PATH, r[0])
...
async def create_order(user_id, order_type, payment_type, details, total_price, contact_info="", delivery_info=""):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        c = await db.execute(
            "INSERT INTO orders (user_id,order_type,payment_type,details,total_price,contact_info,delivery_info) "
            "VALUES (?,?,?,?,?,?,?)",
            (user_id, order_type, payment_type, details, total_price, contact_info, delivery_info),
        )
        await db.commit()
        return c.lastrowid
...
async def get_orders_by_user(user_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        c = await db.execute("SELECT * FROM orders WHERE user_id=? ORDER BY created_at DESC", (user_id,))
        return [dict(r) for r in await c.fetchall()]
...
async def get_all_orders(status_filter=None):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        if status_filter:
            c = await db.execute("SELECT * FROM orders WHERE status=? ORDER BY created_at DESC", (status_filter,))
        else:
...
async def log_message(user_id, direction, content_type, message_text):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO message_log (user_id,direction,content_type,message_text) VALUES (?,?,?,?)",
            (user_id, direction, content_type, (message_text or "")[:500]),
        )
        await db.commit()
...
async def get_stats():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        for key, q in [
            ...
            ("diagnostics", "SELECT COUNT(*) FROM orders WHERE status='diagnostics'"),
            ("in_progress", "SELECT COUNT(*) FROM orders WHERE status='in_progress'"),
            ("ready", "SELECT COUNT(*) FROM orders WHERE status='ready'"),
            ("delayed", "SELECT COUNT(*) FROM orders WHERE status='delayed'"),
            ("waiting_parts", "SELECT COUNT(*) FROM orders WHERE status='waiting_parts'"),
            ("delivery_count", "SELECT COUNT(*) FROM orders WHERE status='delivery'"),
            ("completed", "SELECT COUNT(*) FROM orders WHERE status='completed'"),
            ("cancelled", "SELECT COUNT(*) FROM orders WHERE status='cancelled'"),
            ("portfolio_count", "SELECT COUNT(*) FROM portfolio"),
            ("total_messages", "SELECT COUNT(*) FROM message_log"),
            ("active_today", "SELECT COUNT(*) FROM users WHERE last_active>=datetime('now','-1 day')"),
        ]:
            c = await db.execute(q)
            results[key] = (await c.fetchone())[0]
        return results
...
def get_start_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Наши услуги и цены", url=TAPLINK_URL)],
    ])
...
async def cb_stats(callback: CallbackQuery):
    ...
    t = (
        f"Статистика\n\n"
        f"Пользователи: {s['total_users']}\nАктивны 24ч: {s['active_today']}\n\n"
        f"Заказы: {s['total_orders']}\nВ обработке: {s['processing']}\n"
        f"В работе: {s['in_progress']}\nГотовы: {s['ready']}\n"
        f"Завершены: {s['completed']}\n\n"
        f"Портфолио: {s['portfolio_count']}\nСообщений: {s['total_messages']}"
    )
    try:
